...
const IbWithdrawalStatus = () => {
  ...
  const getCustomContent = (actionType, selectedDeposit, comment) => {
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>IB Withdrawal ${isApprove ? "Success" : "Rejected"}</h1>
        </div>
        <div class="content">
          <p>Dear ${
          ...
          <p>
            ${
              isApprove
                ...
                : "We regret to inform you that your IB withdrawal request has been rejected."
            }
          </p>
  ...
  const handleConfirmAction = async (selectedDeposit) => {
    ...
    try {
      if (actionType === "approve") {
        await backendApi.post(`/withdraw-ib-balance`, {
          referralAccount: selectedDeposit.referralId,
          amount: selectedDeposit.amount,
        });
        setIsDialogOpen(false);
        await backendApi.put(`/update-referral-withdrawal`, {
        ...
        await backendApi.post(`/custom-mail`, {
          ...
          subject: "IB Withdrawal Rejected",
        });
        const updatedDepositData = depositData.map((deposit) =>
          deposit._id === selectedDeposit._id
